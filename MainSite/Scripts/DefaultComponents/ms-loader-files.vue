<template>
  <div>
    ...
    <div class="uploadedFiles">
          ...
          <div class="uploadedFiles__btn-icone">
            <div class="">
              ...
              >
                <path
                  fill-rule="evenodd"
                  d="M6.635 11.772A1 1 0 015 11V5a1 1 0 011.635-.772l3.954 2.964a.999.999 0 010 1.616l-3.954 2.964z"
                ></path>
              </svg>
            </div>
          </div>
      ...
      <div
        class="uploadedFiles_previous uploadedFiles_hidden"
        @click="previousFiles"
      >
          ...
          <div class="uploadedFiles__btn-icone">
            <div class="">
              ...
              >
                <path
                  fill-rule="evenodd"
                  d="M9.365 4.228A1 1 0 0111 5v6a1 1 0 01-1.635.772L5.411 8.808a.999.999 0 010-1.616l3.954-2.964z"
                ></path>
              </svg>
            </div>
          </div>
    ...
    <div style="font-size: 14px" v-if="files.length">
      Количество файлов: {{ files.length }}, Общий размер:
      {{ getAllSizeFiles }}
      <a href="" @click.prevent="deleteAllFiles">удалить файлы</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    transformUploadedFileList(value) {
      let blockUploadedFilesList =
        document.getElementsByClassName("uploadedFiles_list")[0];
      blockUploadedFilesList.style.transform = `translateX(${
      ...
      if (this.translateValue < 0) this.visibleBtn(this.btnPrevious);
      if (
        (blockUploadedFilesList.scrollWidth -
          blockUploadedFilesList.offsetWidth) *
          -1 >=
        this.translateValue
      ) {
    ...
    deleteFile(fileItem) {
      ...
      if (this.files.length <= maxCountItem) {
        ...
        blockUploadedFilesList.style.transform = `translateX(${this.translateValue}px)`;
      } else {
        if (this.translateValue != 0) {
          blockUploadedFilesList.style.transform = `translateX(${
            this.translateValue + 140
          }px)`;
  ...
  mounted() {
    ...
    this.btnPrevious = document.getElementsByClassName(
      "uploadedFiles_previous"
    )[0];
