...
export default {
  ...
  computed: {
    getImgList() {
      if (
        ...
        this.imgList[this.currentIndex] !== undefined
      ) {
        this.displayArrowImgList = {};
        return (
          <MsImageList array={this.imgList} currentIndex={this.currentIndex} />
        );
      }
  ...
  methods: {
    ...
    getClassArrow(callback, prevClass) {
      return `${prevClass} ${
        callback(prevClass) ? prevClass + "_desactive" : ""
      }`;
    },
  },
  created() {
    this.imgList =
      ImageGlobalService.getCurrentImageCalendarService().getData();
    this.currentIndex =
      ImageGlobalService.getCurrentImageCalendarService().getCurrentIndex();
  },
  beforeDestroy() {
