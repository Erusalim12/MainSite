<template>
  ...
  >
    ...
    <div class="s12 m12">
      ...
      <div class="error" style="font-size: 14px" v-if="isErrorHeader">
        Вы должны вести заголовок сообщения
      </div>
      <input
        ref="formTitle"
        name="Header"
        ...
        if (this.isAdvancedEditor) {
          ...
          for (var i = 0; i < this.fileList.length; i++) {
            if (typeof this.fileList[i].dataBaseName == 'undefined') {
              ...
              let dataBaseName =
                typeof this.fileList[i].dataBaseName != 'undefined'
                  ? this.fileList[i].dataBaseName
                  : this.GenerateUUID();
              let file = new File([], this.fileList[i].name + this.fileList[i].extension, {
                type: this.fileList[i].mimeType,
                name: dataBaseName,
              });
