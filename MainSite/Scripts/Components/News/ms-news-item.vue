    ...
    <template v-if="!isEditer">
      <div class="card_news card_news-details">
        ...
        <div class="card_news-editor">
          <a href="#" v-on:click.prevent="changeSectionEditer" title="Редактировать"
            ><i class="material-icons">edit</i></a
          >
          <a href="#" class="error" v-on:click.prevent="deleteNews" title="Удалить"
      ...
      <ul class="dropdownFiles">
        <li v-for="item in news_item.files" :key="item.id">
          <a :href="hrefFile(item.id)"
            ><img src="/images/layout_icons/News/fileLoad.svg" style="padding-right: 5px" />
            {{ item.name }}</a
          >
        </li>
      </ul>
